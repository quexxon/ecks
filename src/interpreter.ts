...
export default class Interpreter {
    ...
    #evaluate(expression: Expression): TypedValue {
        switch (expression.kind) {
            case "primitive":
                return this.#primitive(expression);
            case "grouping":
                return this.#grouping(expression);
            case "unary":
            ...
            case "cond":
                return this.#cond(expression);
            case "case":
                return this.#case(expression);
            case "let":
                return this.#let(expression);
            case "array":
            ...
            case "method-call":
                return this.#methodCall(expression);
            case "index":
    ...
    #unary(unary: Unary): TypedValue {
        ...
        interface UnaryOperand {
            [key: symbol]: () => TypedValue;
        }

        if (unary.operator === UnaryOperator.Negation) {
            if (Symbol.for("neg") in operand) {
                return (operand as unknown as UnaryOperand)[Symbol.for("neg")]();
            }
        }

        if (Symbol.for(unary.operator) in operand) {
            return (operand as unknown as UnaryOperand)[Symbol.for(unary.operator)]();
        }
    ...
    #binary(binary: Binary): TypedValue {
        ...
        interface BinaryOperand {
            [key: symbol]: (r: TypedValue) => TypedValue;
        }

        if (Symbol.for(binary.operator) in l) {
            return (l as unknown as BinaryOperand)[Symbol.for(binary.operator)](r);
        }
    ...
    #array(array: ArrayGroup): TypedValue {
        return new XArray(
            array.elements.map((e) => this.#evaluate(e)),
            this.#state
        );
    }
    ...
    #set(set: SetGroup): TypedValue {
        return new XSet(
            set.elements.map((e) => this.#evaluate(e)),
            this.#state
        );
    }
    ...
    #tuple(tuple: TupleGroup): TypedValue {
        return new XTuple(
            tuple.elements.map((e) => this.#evaluate(e)),
            this.#state
        );
    }
    ...
    #record(record: RecordGroup): TypedValue {
        ...
        return new RecordType(
            record.members.reduce<Map<string, TypedValue>>(
                (members, [id, expr]) => {
    ...
    #methodCall(methodCall: MethodCall): TypedValue {
        ...
        const method = (receiver as any)[identifier]; // eslint-disable-line @typescript-eslint/no-explicit-any
        if (typeof method === "undefined") {
            throw new TypeError(
                `No method "${identifier}" for ${receiver.kind}`
            );
        }
        ...
        if (typeof method === "function") {
            return method.apply(
                receiver,
                methodCall.arguments.map((exp) => this.#evaluate(exp))
            );
        }
        return method;
    }
    ...
    #index(index: Index): TypedValue {
        ...
        throw new TypeError(
            `Index expressions are not supported for ${receiver.kind}`
        );
    }

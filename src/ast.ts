...
export function toString(expr: Expression): string {
    switch (expr.kind) {
        ...
        case "binary":
            return `${toString(expr.left)} ${expr.operator} ${toString(
                expr.right
            )}`;
        case "ternary":
            return `${toString(expr.antecedent)} ? ${toString(
                expr.consequent
            )} : ${toString(expr.alternative)}`;
        case "cond": {
            const branches = expr.branches
                ...
                .join(", ");
            const elseBranch =
                expr.else === undefined ? "" : `else: ${toString(expr.else)}`;
            let body = "";
        ...
        case "method-call": {
            ...
            return `${toString(expr.receiver)}.${toString(
                expr.identifier
            )}${args}`;
        }
        case "index":
        ...
        case "lambda":
            return `|${expr.parameters.map(toString).join(" ")}| ${toString(
                expr.body
            )}`;
        case "optional":

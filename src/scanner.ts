...
export default class Scanner {
    ...
    #scanToken(): void {
        ...
        switch (char) {
            ...
            case ":":
                ...
                break;
            case "&":
                this.#addToken(TokenKind.Ampersand);
                break;
            case "%":
                this.#addToken(TokenKind.Percent);
                break;
            ...
            case "!":
                this.#addToken(
                    this.#isMatch("=") ? TokenKind.BangEqual : TokenKind.Bang
                );
                break;
            case ">":
                this.#addToken(
            ...
            case "<":
                this.#addToken(
                    this.#isMatch("=") ? TokenKind.LessEqual : TokenKind.Less
                );
                break;
            case "?":
                this.#addToken(
    ...
    #scanString(quoteChar: string): void {
        ...
        while (
            (this.#peek() !== quoteChar || inExpression) &&
            !this.#isAtEnd()
        ) {
            if (isEscape) {
                isEscape = false;
    ...
    #scanIdentifier(): void {
        ...
        const identifier: string = this.#source
            .slice(this.#start, this.#current)
            .toLowerCase();

        switch (identifier) {
            case "and":
                this.#addToken(TokenKind.And);
                break;
            case "or":
                this.#addToken(TokenKind.Or);
                break;
            case "case":
                this.#addToken(TokenKind.Case);
                break;
            case "cond":
                this.#addToken(TokenKind.Cond);
                break;
            case "else":
                this.#addToken(TokenKind.Else);
                break;
            case "let":
                this.#addToken(TokenKind.Let);
                break;
            case "true":
                this.#addToken(TokenKind.True, true);
                break;
            case "false":
                this.#addToken(TokenKind.False, false);
                break;
            case "some":
                this.#addToken(TokenKind.Some);
                break;
            case "none":
                this.#addToken(TokenKind.None);
                break;
            default:

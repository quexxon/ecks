...
export default class XArray {
    ...
    at(index: TypedValue): XOptional {
        ...
        const value =
            index.__value >= 0 && index.__value < this.#value.length
    ...
    mapi(lambda: XLambda): XArray {
        return this.__new(
            this.#value.map((v, i) =>
                lambda.call(v, new XInteger(i, this.#state))
            )
        );
    }
    ...
    sort(): XArray {
        return this.__new(
            Array.from(this.#value).sort((x, y) => {
    ...
    [Symbol.for("+")](value: TypedValue): XArray {
        ...
        if (value.__value[0].kind !== this.#value[0].kind)
            throw new TypeError();

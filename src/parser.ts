...
export default class Parser {
    ...
    #ternary(): Expression {
        ...
        if (this.#match(TokenKind.Question)) {
            ...
            this.#consume(
                TokenKind.Colon,
                "Expected `:` between consequent and alternative"
            );
    ...
    #factor(): Expression {
        ...
        while (
            this.#match(TokenKind.Slash, TokenKind.Star, TokenKind.Percent)
        ) {
    ...
    #method(): Expression {
        ...
        while (
            ...
            this.#peek()?.kind === TokenKind.LeftBracket
        ) {
    ...
    #compound(): Expression {
        if (this.#match(TokenKind.Cond)) {
            ...
            while (!this.#match(TokenKind.RightBrace)) {
                ...
                this.#consume(
                    TokenKind.Colon,
                    "Expected `:` between antecedent and consequent"
                );
                const consequent = this.#expression();
                branches.push([antecedent, consequent]);
                this.#match(TokenKind.Comma); // skip optional comma
                if (this.#isAtEnd()) {
                    throw new UnmatchedOpeningChar(
                        "Expected closing `}` in `cond` expression"
                    );
                }
            }
        ...
        if (this.#match(TokenKind.Let)) {
            ...
            while (!this.#match(TokenKind.RightBrace)) {
                ...
                if (this.#isAtEnd()) {
                    throw new UnmatchedOpeningChar(
                        "Expected closing `}` in `let` expression"
                    );
                }
            }
    ...
    #primary(): Expression {
        ...
        if (this.#match(TokenKind.LeftBrace)) {
            ...
            while (!this.#match(TokenKind.RightBrace)) {
                ...
                if (this.#isAtEnd()) {
                    throw new UnmatchedOpeningChar(
                        "Expected closing `}` in map literal"
                    );
                }
        ...
        if (this.#match(TokenKind.Bar)) {
            ...
            if (this.#isAtEnd()) {
                throw new UnexpectedEof(
                    "Expected expression after lambda params"
                );
            }
            const body = this.#expression();
